...
int main() {     
    ...
    //cout<<endl;
    n = a.size();
    vector<vector<int> >co(n,vector<int>(n*n/4));
    do{
        int ans = fw(a);
        #ifdef printa
        cout<<'[';
        for(int i= 1;i<n - 3;i++)cout<<a[i]<<(a[i]<a[i+1]?", ":"; ");
        cout<<a[n-3]<<"]: ";
        cout<<ans<<endl;
        #endif

       int desc = 0;
       for(int i= 0;i<n-2;i++)if(a[i] >a[i+1])desc++;
        co[desc][ans]++;
    }while(next_permutation(a.begin()+1,a.end()-2));
    for(int i= 0;i<co.size();i++)
    {
        for(int j= 0;j<co[0].size();j++)
        {
            if(co[i][j] >= 1000000)cout<<co[i][j]<<" ";
            else if(co[i][j] >= 100000)cout<<co[i][j]<<"  ";
            else if(co[i][j] >= 10000)cout<<co[i][j]<<"   ";
            else if(co[i][j] >= 1000)cout<<co[i][j]<<"    ";
            else if(co[i][j]>=100)cout<<co[i][j]<<"     ";
            else if(co[i][j] >= 10)cout<<co[i][j]<<"      ";
            else cout<<co[i][j]<<"       ";
        }
        cout<<endl;
    }
